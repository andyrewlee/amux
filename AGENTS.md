...
- Log levels: `logging.Error` = a user-visible operation failed and the app continued (shelve failed, kill failed, persist failed, send failed — the user asked for something and it did not happen). `logging.Warn` = degraded, retrying, or self-healing (GC sweeps, discovery retries, fallbacks, input-validation rejections). Debug/Info are unchanged verbosity tiers. The boundary is "user-initiated op vs background self-heal" — `AMUX_LOG_LEVEL=error` must surface the failures a user needs to see.
- Docs-drift checklist: when a lifecycle, keybinding, config key, env var, or tmux tag/name surface changes, update `README.md`, `docs/CONFIG.md`, and `docs/ORCHESTRATION.md` in the same change — they are the user-facing contract.
