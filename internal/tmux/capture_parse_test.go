...
// These tests exercise the pure parser split out of paneSize. The
// malformed/zero-or-negative error branches guard against tmux emitting
// non-numeric or non-positive fields; a live tmux server never produces such
// output, so before the split these branches were unreachable from any
// integration test and silently uncovered. Driving the parser with crafted
// lines lets us pin the field-count, paneID-match, strconv and validation
// behavior without a subprocess.
