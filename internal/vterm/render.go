...
func (v *VTerm) renderScreenCached(screen [][]Cell) string {
	...
	cursorHidden := v.CursorHiddenForRender()

	// Invalidate cursor lines if cursor state changed
	if v.ShowCursor != v.lastShowCursor || cursorHidden != v.lastCursorHidden || v.CursorX != v.lastCursorX || v.CursorY != v.lastCursorY {
		...
		v.lastCursorHidden = cursorHidden
