...
func waitForGitPath(path string, timeout time.Duration) error {
	...
	for {
		...
		time.Sleep(gitPathPollInterval)
	}
}
...
// gitPathPollInterval is the polling interval when waiting for a new worktree
// to expose .git.
const gitPathPollInterval = 100 * time.Millisecond
