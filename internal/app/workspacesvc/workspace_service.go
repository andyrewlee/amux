	...
	return func() tea.Msg {
		...
		if err := validation.ValidateProjectPath(path); err != nil {
			...
			return messages.Error{
				Err:     err,
				Context: workspaceErrContext("adding project"),
			}
		}
		...
		if s == nil || s.registry == nil {
			return messages.Error{Err: errors.New("registry unavailable"), Context: workspaceErrContext("adding project")}
		}

		// Add to registry
		if err := s.registry.AddProject(path); err != nil {
			logging.Error("Failed to add project to registry: %v", err)
			return messages.Error{Err: err, Context: workspaceErrContext("adding project")}
		}
...
func (s *Service) DeleteWorkspace(project *data.Project, ws *data.Workspace) tea.Cmd {
	...
	return func() tea.Msg {
		wsID := string(ws.ID())
		// Stamp the identity keys now, while the worktree still exists and
		// ws.ID() still equals the resolved form the UI keyed its state under —
		// recomputing post-removal yields the unresolved form and misses them.
		...
		projectPath, failMsg := s.validateWorkspaceDeleteInputs(project, ws, fail)
		if failMsg != nil {
			return failMsg
		}
...
// validateWorkspaceDeleteInputs runs the pre-delete checks that reject a
// delete before any destructive step. It returns the normalized project path
// for the caller's later stages, or a fail message to return immediately.
func (s *Service) validateWorkspaceDeleteInputs(
	project *data.Project,
	ws *data.Workspace,
	fail func(string, error) tea.Msg,
) (string, tea.Msg) {
...
func (s *Service) killWorkspaceSessionsForDelete(wsID string) error {
...
func (s *Service) archiveDeletedWorkspaceMetadata(ws *data.Workspace) error {
...
func (s *Service) removeWorktreeAndBranchLocked(
...
func (s *Service) handleStaleRemoveError(
