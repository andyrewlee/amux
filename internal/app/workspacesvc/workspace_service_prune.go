...
// workspaceRepoMetadataDeleter is the optional bulk-delete capability of a
// real WorkspaceStore: one DeleteByRepo call replaces a list+per-record Delete
// loop. Fakes implementing only WorkspaceStore take the compatibility path
...
func (s *Service) reconcileWorkspaceMetadata(registeredRepos []string) {
	if s == nil || s.store == nil {
		return
	}
	result, err := s.store.PruneStale(data.WorkspacePruneOptions{
...
func (s *Service) pruneMissingTemporaryProjects(paths []string) []string {
...
func (s *Service) removeProjectMetadata(repoPath string, knownWorkspaces ...data.Workspace) {
	if s == nil {
		return
	}
...
func (s *Service) restoreMetadataIfProjectReadded(repoPath string) {
	if s == nil || s.registry == nil {
		return
	}
...
const metadataOrphanGracePeriod = time.Hour
...
// This is the ACCIDENTAL-archive window only: intentionally shelved
// workspaces (ws.Shelved) are exempt — the user asked for the shelf, so
// only an explicit purge removes it.
const archivedMetadataRetention = 7 * 24 * time.Hour
