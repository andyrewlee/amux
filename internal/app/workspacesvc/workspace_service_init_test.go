...
// thin adapter New wires in. It asserts the worktree is
...
// fakeAssistantStore intentionally stays local rather than using
// testutil.FakeWorkspaceStore: its contract is *strictness* — every method but
// ResolvedDefaultAssistant panics, proving the init path touches nothing else.
// The shared fake's silent zero-value methods would hide an unexpected call.
type fakeAssistantStore struct {
	assistant string
}

func (f *fakeAssistantStore) MarkDeleting(data.WorkspaceID) error  { return nil }
func (f *fakeAssistantStore) IsDeleting(data.WorkspaceID) bool     { return false }
func (f *fakeAssistantStore) ClearDeleting(data.WorkspaceID) error { return nil }
func (f *fakeAssistantStore) PruneStale(data.WorkspacePruneOptions) (data.WorkspacePruneResult, error) {
...
func (f *fakeAssistantStore) Load(data.WorkspaceID) (*data.Workspace, error) {
	return nil, nil
}
...
func TestWorkspaceServiceResolvedDefaultAssistant(t *testing.T) {
	...
	t.Run("delegates to the wired store", func(t *testing.T) {
		svc := &Service{store: &fakeAssistantStore{assistant: "codex"}}
...
// TestWorkspaceServiceRunUnlessMutationInFlightGuards covers the two branches the
...
func TestWorkspaceServiceRunUnlessMutationInFlightGuards(t *testing.T) {
	var nilService *Service
	if nilService.runUnlessMutationInFlight(&data.Workspace{Root: "/r/ws"}, func() {}) {
		t.Fatal("nil service should report the callback as not run")
	}

	svc := &Service{}
	if !svc.runUnlessMutationInFlight(&data.Workspace{Root: "/r/ws"}, nil) {
...
func TestNewWorkspaceServiceWiring(t *testing.T) {
	...
	svc := New(nil, store, nil, "/tmp/workspaces")

	if svc == nil {
		t.Fatal("New returned nil")
	}
	if svc.gitOps == nil {
		t.Fatal("New must wire a non-nil GitOperations adapter")
	}
	if _, ok := svc.gitOps.(defaultGitOps); !ok {
		t.Fatalf("New gitOps = %T, want defaultGitOps", svc.gitOps)
	}
	if svc.gitPathWaitTimeout != 3*time.Second {
		t.Fatalf("gitPathWaitTimeout = %v, want %v", svc.gitPathWaitTimeout, 3*time.Second)
	}
	if svc.store != store {
		t.Fatalf("New stored store %p, want %p", svc.store, store)
	}
...
func (f *fakeAssistantStore) SetScripts(data.WorkspaceID, data.ScriptsConfig, string) error {
	return nil
}
