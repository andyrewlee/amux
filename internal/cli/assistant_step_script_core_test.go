...
func TestAssistantStepScriptRun_RecoversTimedOutNoOutputFromCapture(t *testing.T) {
	requireBinary(t, "jq")
...
case "${1:-} ${2:-}" in
  "agent run")
    ...
    ;;
  "agent send")
    printf '%s' "${FAKE_AMUX_SEND_JSON:-${FAKE_AMUX_RUN_JSON:?missing FAKE_AMUX_SEND_JSON}}"
    ;;
  "agent capture")
    printf '%s' "${FAKE_AMUX_CAPTURE_JSON:?missing FAKE_AMUX_CAPTURE_JSON}"
    ;;
  *)
...
`), 0o755); err != nil {
	...
	env = withEnv(env, "FAKE_AMUX_CAPTURE_JSON", captureJSON)
	env = withEnv(env, "AMUX_ASSISTANT_STEP_TIMEOUT_RECOVERY_POLLS", "1")
	env = withEnv(env, "AMUX_ASSISTANT_STEP_TIMEOUT_RECOVERY_INTERVAL", "0")
	env = withEnv(env, "AMUX_ASSISTANT_STEP_TIMEOUT_RECOVERY_LINES", "80")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
	...
	if chunkChars, _ := channel["chunk_chars"].(float64); chunkChars != 1200 {
		t.Fatalf("assistant.chunk_chars = %v, want 1200", chunkChars)
	}
	msg, _ := channel["message"].(string)
	if !strings.Contains(msg, "Recovered status update") {
		t.Fatalf("assistant.message = %q, expected summary text", msg)
	}
	...
	if got, _ := channel["inline_buttons_scope"].(string); got != "allowlist" {
		t.Fatalf("assistant.inline_buttons_scope = %q, want %q", got, "allowlist")
	}
...
func TestAssistantStepScriptParseError_MissingFlagValueReturnsJSON(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantStepScriptRun_SetsBlockedPermissionMode(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantStepScriptRun_AutoIdempotencyCanBeDisabled(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`), 0o755); err != nil {
	...
	env = withEnv(env, "PATH", fakeBinDir+":"+os.Getenv("PATH"))
	env = withEnv(env, "FAKE_AMUX_RUN_JSON", runJSON)
	env = withEnv(env, "AMUX_ASSISTANT_STEP_AUTO_IDEMPOTENCY", "false")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("assistant-step.sh run failed: %v", err)
...
func TestAssistantStepScriptRun_HardTimeoutNeverUndercutsWaitTimeout(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`), 0o755); err != nil {
	...
	runJSON := `{"ok":true,"data":{"session_name":"sess-hard-timeout","agent_id":"agent-hard-timeout","workspace_id":"ws-hard-timeout","assistant":"codex","response":{"status":"idle","latest_line":"done","summary":"done","delta":"done","needs_input":false,"input_hint":"","timed_out":false,"session_exited":false,"changed":true}}}`

	cmd := exec.Command(
		scriptPath,
		"run",
		"--workspace", "ws-hard-timeout",
		"--assistant", "codex",
		"--prompt", "test prompt",
		"--wait-timeout", "3s",
		"--idle-threshold", "1s",
	...
	env = withEnv(env, "PATH", fakeBinDir+":"+os.Getenv("PATH"))
	env = withEnv(env, "FAKE_AMUX_RUN_JSON", runJSON)
	env = withEnv(env, "AMUX_ASSISTANT_STEP_HARD_TIMEOUT_BUFFER", "0")
	env = withEnv(env, "AMUX_ASSISTANT_STEP_HARD_TIMEOUT_CAP", "1")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("assistant-step.sh run failed: %v", err)
...
func TestAssistantStepScriptRun_UpgradesWeakSummaryFromDelta(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
