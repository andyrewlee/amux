...
func buildSetupCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, providerName, err := computer.ResolveProvider(cfg, "", computer.ProviderDaytona)
			if err != nil {
				return err
			}
			if providerName != computer.ProviderDaytona {
				return fmt.Errorf("setup currently supports only provider %q", computer.ProviderDaytona)
			}
			...
			if createSnapshot {
				parsedAgents, err := computer.ParseAgentList(agents)
				if err != nil {
					return err
				}
				if baseImage == "" {
				...
				snap, err := computer.CreateSnapshot(client, name, parsedAgents, baseImage, func(chunk string) {
					fmt.Println(chunk)
				})
				if err != nil {
					return err
...
func agentsToStrings(agents []computer.Agent) []string {
...
func joinAgents(agents []computer.Agent) string {
