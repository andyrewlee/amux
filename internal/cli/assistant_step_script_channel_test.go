...
func TestAssistantStepScriptRun_QuietVerbositySuppressesDetailSections(t *testing.T) {
	requireBinary(t, "jq")
...
`), 0o755); err != nil {
	...
	env = withEnv(env, "AMUX_ASSISTANT_STEP_VERBOSITY", "quiet")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
	...
	if strings.Contains(msg, "Details:") || strings.Contains(msg, "Command:") || strings.Contains(msg, "Next:") {
		t.Fatalf("assistant.message should suppress detail sections in quiet mode: %q", msg)
	}
}

func TestAssistantStepScriptRun_DisablesInlineButtonsWhenScopeOff(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantStepScriptRun_AddsChunkContinuationMetadata(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`), 0o755); err != nil {
	...
	runJSON := `{"ok":true,"data":{"session_name":"sess-4","agent_id":"agent-4","workspace_id":"ws-4","assistant":"codex","response":{"status":"idle","latest_line":"done","summary":"This is a very long summary intended to force Assistant chunking into multiple segments with continuation metadata for better mobile readability.","delta":"line1\nline2\nline3\nline4\nline5\nline6","needs_input":false,"input_hint":"","timed_out":false,"session_exited":false,"changed":true}}}`
	...
	env = withEnv(env, "PATH", fakeBinDir+":"+os.Getenv("PATH"))
	env = withEnv(env, "FAKE_AMUX_RUN_JSON", runJSON)
	env = withEnv(env, "AMUX_ASSISTANT_STEP_CHUNK_CHARS", "80")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("assistant-step.sh run failed: %v", err)
	...
	if !ok || len(chunks) < 2 {
		t.Fatalf("assistant.chunks expected at least 2 chunks: %#v", channel["chunks"])
	}
	...
	if !ok || len(chunksMeta) != len(chunks) {
		t.Fatalf("assistant.chunks_meta mismatch: chunks=%d meta=%#v", len(chunks), channel["chunks_meta"])
	}
}

func TestAssistantStepScriptRun_UsesAbsoluteSelfPathInSuggestedCommand(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")

	relScriptPath := filepath.Join("..", "..", "skills", "amux", "scripts", "assistant-step.sh")
