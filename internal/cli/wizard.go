...
func RunFirstRunWizard() (*WizardConfig, error) {
	...
	fmt.Println("Let's set up your computer environment.")
	fmt.Println()
	...
	fmt.Println("Select the AI coding agents you want to use in computers.")
	fmt.Println()
	...
	fmt.Println("\033[1m[2/3] Sync local settings to computer?\033[0m")
	fmt.Println("This copies your preferences (NOT credentials) to the computer.")
	fmt.Println()

	config.SyncSettings = confirm("Enable settings sync?")
	if config.SyncSettings {
		// Check which settings exist locally
		status := computer.GetLocalSettingsStatus()

		if status[computer.AgentClaude] {
...
func ApplyWizardConfig(config *WizardConfig) error {
	...
	if config.SyncSettings {
		syncCfg := computer.SettingsSyncConfig{
			Enabled: true,
			Claude:  config.SyncClaude,
			Git:     config.SyncGit,
		}

		if err := computer.SaveSettingsSyncConfig(syncCfg); err != nil {
	...
	for _, agent := range config.Agents {
		fmt.Printf("  amux %s    # Run %s in a computer\n", agent, agent)
	}
	fmt.Println()
	fmt.Println("Other commands:")
	fmt.Println("  amux status     # Check computer status")
