...
func resolveSandboxHome(sb *daytona.Sandbox) string {
	resp, err := sb.Process.ExecuteCommand(`sh -lc "USER_NAME=$(id -un 2>/dev/null || echo daytona); HOME_DIR=$(getent passwd \"$USER_NAME\" 2>/dev/null | cut -d: -f6 || true); if [ -z \"$HOME_DIR\" ]; then HOME_DIR=/home/$USER_NAME; fi; printf \"%s\" \"$HOME_DIR\""`)
	if err == nil {
		if resp.Artifacts != nil && resp.Artifacts.Stdout != "" {
			return strings.TrimSpace(resp.Artifacts.Stdout)
		}
		if resp.Result != "" {
