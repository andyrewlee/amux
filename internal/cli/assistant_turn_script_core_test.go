...
func TestAssistantTurnScript_CompletesAfterFollowupStep(t *testing.T) {
	requireBinary(t, "jq")
...
`), 0o755); err != nil {
	...
	if !ok || len(chunks) == 0 {
		t.Fatalf("assistant.chunks missing or empty: %#v", channel["chunks"])
	}
	chunksMeta, ok := channel["chunks_meta"].([]any)
	if !ok || len(chunksMeta) == 0 {
		t.Fatalf("assistant.chunks_meta missing or empty: %#v", channel["chunks_meta"])
	}
	...
	if !ok || len(channelProgressUpdates) != 2 {
		t.Fatalf("assistant.progress_updates = %#v, want len=2", channel["progress_updates"])
	}
	if got, _ := channel["verbosity"].(string); got != "normal" {
		t.Fatalf("assistant.verbosity = %q, want %q", got, "normal")
	}
	if got, _ := channel["inline_buttons_scope"].(string); got != "allowlist" {
		t.Fatalf("assistant.inline_buttons_scope = %q, want allowlist", got)
	}
	if got, _ := channel["inline_buttons_enabled"].(bool); !got {
		t.Fatalf("assistant.inline_buttons_enabled = %v, want true", got)
	}
	if got, _ := channel["callback_data_max_bytes"].(float64); got != 64 {
		t.Fatalf("assistant.callback_data_max_bytes = %v, want 64", got)
	}
	inlineButtons, ok := channel["inline_buttons"].([]any)
	if !ok || len(inlineButtons) == 0 {
		t.Fatalf("assistant.inline_buttons missing or empty: %#v", channel["inline_buttons"])
	}
	actionTokens, ok := channel["action_tokens"].([]any)
	if !ok || len(actionTokens) == 0 {
		t.Fatalf("assistant.action_tokens missing or empty: %#v", channel["action_tokens"])
	}
	actionsFallback, _ := channel["actions_fallback"].(string)
	if !strings.Contains(actionsFallback, "qa:") {
		t.Fatalf("assistant_ux.actions_fallback = %q, expected qa: token list", actionsFallback)
	}
}

func TestAssistantTurnScriptParseError_MissingFlagValueReturnsJSON(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantTurnScriptSend_AllowsDashPrefixedTextValue(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantTurnScript_CoalescesDuplicateMilestonesAndStopsOnTimeoutStreak(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantTurnScript_QuietVerbositySuppressesExtraSections(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`), 0o755); err != nil {
	...
	env = withEnv(env, "FAKE_STEP_1_JSON", step1)
	env = withEnv(env, "AMUX_ASSISTANT_TURN_STEP_SCRIPT", fakeStepPath)
	env = withEnv(env, "AMUX_ASSISTANT_TURN_VERBOSITY", "quiet")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("assistant-turn.sh run failed: %v", err)
	...
	if strings.Contains(msg, "Next:") || strings.Contains(msg, "Command:") || strings.Contains(msg, "Meta:") || strings.Contains(msg, "Progress:") {
		t.Fatalf("assistant.message should suppress extra sections in quiet mode: %q", msg)
	}
}

func TestAssistantTurnScript_DisablesInlineButtonsWhenScopeOff(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`), 0o755); err != nil {
	...
	step1 := `{"ok":true,"mode":"run","status":"needs_input","summary":"Need approval to continue.","agent_id":"agent-inline-off","workspace_id":"ws-inline-off","assistant":"codex","response":{"substantive_output":true,"needs_input":true},"next_action":"Choose A or B.","suggested_command":"skills/amux/scripts/assistant-step.sh send --agent agent-inline-off --text \"A\" --enter --wait-timeout 60s --idle-threshold 10s"}`
	...
	env = withEnv(env, "FAKE_STEP_1_JSON", step1)
	env = withEnv(env, "AMUX_ASSISTANT_TURN_STEP_SCRIPT", fakeStepPath)
	env = withEnv(env, "AMUX_ASSISTANT_INLINE_BUTTONS_SCOPE", "off")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("assistant-turn.sh run failed: %v", err)
	...
	if got, _ := channel["inline_buttons_scope"].(string); got != "off" {
		t.Fatalf("assistant.inline_buttons_scope = %q, want off", got)
	}
	if got, _ := channel["inline_buttons_enabled"].(bool); got {
		t.Fatalf("assistant.inline_buttons_enabled = %v, want false", got)
	}
	if inlineButtons, ok := channel["inline_buttons"].([]any); !ok || len(inlineButtons) != 0 {
		t.Fatalf("assistant.inline_buttons = %#v, want empty", channel["inline_buttons"])
	}
	quickActions, ok := payload["quick_actions"].([]any)
	if !ok || len(quickActions) == 0 {
		t.Fatalf("quick_actions missing or empty: %#v", payload["quick_actions"])
