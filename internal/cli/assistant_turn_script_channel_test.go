...
func TestAssistantTurnScript_AddsChunkContinuationMetadata(t *testing.T) {
	requireBinary(t, "jq")
...
`), 0o755); err != nil {
	...
	step1 := `{"ok":true,"mode":"run","status":"timed_out","summary":"Warming up and gathering repository context.","agent_id":"agent-3","workspace_id":"ws-3","assistant":"codex","response":{"substantive_output":false,"needs_input":false},"next_action":"Continue.","suggested_command":"skills/amux/scripts/assistant-step.sh send --agent agent-3 --text \"Continue\" --enter --wait-timeout 60s --idle-threshold 10s"}`
	step2 := `{"ok":true,"mode":"send","status":"idle","summary":"Implemented a long list of refactors and added tests and docs and validation so the final Assistant update should be split into multiple chunks for readability.","agent_id":"agent-3","workspace_id":"ws-3","assistant":"codex","response":{"substantive_output":true,"needs_input":false},"next_action":"Review patch.","suggested_command":""}`
	...
	env = withEnv(env, "AMUX_ASSISTANT_TURN_CHUNK_CHARS", "80")
	cmd.Env = env
	out, err := cmd.Output()
	if err != nil {
...
func TestAssistantTurnScript_UsesSiblingStepScriptWhenInvokedOutsideRepoRoot(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")

	sourceScriptPath := filepath.Join("..", "..", "skills", "amux", "scripts", "assistant-turn.sh")
	...
	scriptDir := t.TempDir()
	runDir := t.TempDir()
	copiedScriptPath := filepath.Join(scriptDir, "assistant-turn.sh")
	...
	fakeStepPath := filepath.Join(scriptDir, "assistant-step.sh")
	if err := os.WriteFile(fakeStepPath, []byte(`#!/usr/bin/env bash
set -euo pipefail
