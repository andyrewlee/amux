...
func parseVolumeSpec(spec string) (computer.VolumeSpec, error) {
	parts := strings.SplitN(spec, ":", 2)
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		return computer.VolumeSpec{}, fmt.Errorf("invalid volume spec %q. Use name:/path", spec)
	}
	return computer.VolumeSpec{Name: parts[0], MountPath: parts[1]}, nil
}
...
func getAgentArgs(argv []string, agent string) []string {
	...
	for i := 0; i < len(argv)-2; i++ {
		if argv[i] == "computer" && argv[i+1] == "run" && argv[i+2] == agent {
			agentIndex = i + 2
			break
		}
	}
