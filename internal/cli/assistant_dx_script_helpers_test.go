...
var assistantDXContextByTest sync.Map
...
func withIsolatedAssistantDXContext(t *testing.T, env []string) []string {
	t.Helper()
	if envHasKey(env, "AMUX_ASSISTANT_DX_CONTEXT_FILE") {
		lastValue, _ := envLastValue(env, "AMUX_ASSISTANT_DX_CONTEXT_FILE")
		if lastValue != "" && lastValue != os.Getenv("AMUX_ASSISTANT_DX_CONTEXT_FILE") {
			return env
		}
		env = envWithoutKey(env, "AMUX_ASSISTANT_DX_CONTEXT_FILE")
	}
	if existing, ok := assistantDXContextByTest.Load(t.Name()); ok {
		if existingPath, ok := existing.(string); ok && existingPath != "" {
			return append(env, "AMUX_ASSISTANT_DX_CONTEXT_FILE="+existingPath)
		}
	}
	path := filepath.Join(t.TempDir(), "assistant-dx-context.json")
	assistantDXContextByTest.Store(t.Name(), path)
	t.Cleanup(func() {
		assistantDXContextByTest.Delete(t.Name())
	})
	return append(env, "AMUX_ASSISTANT_DX_CONTEXT_FILE="+path)
}
...
func runScriptJSON(t *testing.T, scriptPath string, env []string, args ...string) map[string]any {
	...
	cmd.Env = withIsolatedAssistantDXContext(t, env)
...
func TestRunScriptJSON_AutoInjectsStableAssistantDXContext(t *testing.T) {
	requireBinary(t, "bash")

	scriptPath := filepath.Join(t.TempDir(), "print-context.sh")
	writeExecutable(t, scriptPath, `#!/usr/bin/env bash
set -euo pipefail
printf '{"context":"%s"}' "${AMUX_ASSISTANT_DX_CONTEXT_FILE:-}"
`)
	...
	if got := filepath.Base(firstCtx); got != "assistant-dx-context.json" {
		t.Fatalf("context file basename = %q, want %q", got, "assistant-dx-context.json")
	}
}

func TestRunScriptJSON_RespectsExplicitAssistantDXContext(t *testing.T) {
	requireBinary(t, "bash")
...
`)
	...
	env := append(os.Environ(), "AMUX_ASSISTANT_DX_CONTEXT_FILE="+explicitPath)
...
func TestRunScriptJSON_ReplacesAmbientAssistantDXContext(t *testing.T) {
	requireBinary(t, "bash")
...
`)
	...
	t.Setenv("AMUX_ASSISTANT_DX_CONTEXT_FILE", ambientPath)
	env := os.Environ()
...
func TestRunScriptJSONInDir_AutoInjectsStableAssistantDXContext(t *testing.T) {
	requireBinary(t, "bash")
	...
	writeExecutable(t, scriptPath, `#!/usr/bin/env bash
set -euo pipefail
printf '{"cwd":"%s","context":"%s"}' "$(pwd)" "${AMUX_ASSISTANT_DX_CONTEXT_FILE:-}"
`)
