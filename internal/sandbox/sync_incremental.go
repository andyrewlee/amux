...
func GetRemoteManifest(sandbox *daytona.Sandbox, remotePath string) (*WorkspaceManifest, error) {
	manifestPath := filepath.Join(remotePath, manifestFileName)

	// Try to read the manifest file
	resp, err := sandbox.Process.ExecuteCommand(SafeCommands.Cat(manifestPath))
...
func SaveRemoteManifest(sandbox *daytona.Sandbox, remotePath string, manifest *WorkspaceManifest) error {
	...
	manifestPath := filepath.Join(remotePath, manifestFileName)
	return sandbox.FS.UploadFile(data, manifestPath, 0)
}
...
func IncrementalSync(sandbox *daytona.Sandbox, opts SyncOptions, verbose bool) error {
	...
	remotePath := GetWorkspaceRepoPath(sandbox, opts)

	// Get remote manifest
	logger.Debug("fetching remote manifest")
	remoteManifest, err := GetRemoteManifest(sandbox, remotePath)
	if err != nil {
	...
	if totalFiles > 0 && float64(totalChanges)/float64(totalFiles) > 0.5 {
		...
		return UploadWorkspace(sandbox, opts, verbose)
	}
	...
	_, _ = sandbox.Process.ExecuteCommand(SafeCommands.MkdirP(remotePath))

	// Delete removed files
	for _, path := range diff.Deleted {
		fullPath := filepath.Join(remotePath, path)
		logger.Debug("deleting", "path", path)
		_, _ = sandbox.Process.ExecuteCommand(SafeCommands.RmRf(fullPath))
	}
	...
	for _, path := range filesToUpload {
		...
		_, _ = sandbox.Process.ExecuteCommand(SafeCommands.MkdirP(parentDir))
		...
		if err := sandbox.FS.UploadFile(data, remoteFull, 0); err != nil {
			logger.Warn("failed to upload file", "path", path, "error", err)
			continue
		}
	}
	...
	if err := SaveRemoteManifest(sandbox, remotePath, localManifest); err != nil {
...
func ShouldUseIncrementalSync(sandbox *daytona.Sandbox, opts SyncOptions) bool {
	remotePath := GetWorkspaceRepoPath(sandbox, opts)
	manifestPath := filepath.Join(remotePath, manifestFileName)

	// Check if manifest exists
	resp, err := sandbox.Process.ExecuteCommand(SafeCommands.Test("-f", manifestPath))
	if err != nil || resp.ExitCode != 0 {
...
func SmartSync(sandbox *daytona.Sandbox, opts SyncOptions, verbose bool) error {
	if ShouldUseIncrementalSync(sandbox, opts) {
		err := IncrementalSync(sandbox, opts, verbose)
		if err == nil {
			return nil
		}
