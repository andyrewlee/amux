...
func (e *SandboxError) defaultSuggestion() string {
	switch e.Code {
	case ErrCodeSandboxCreate:
		return "Check your Daytona API key with `amux auth status`. Run `amux doctor` for diagnostics."
	case ErrCodeSandboxNotFound:
	...
	case ErrCodeVolume:
		return "The credentials volume may not exist. Run `amux doctor --deep` to check."
	case ErrCodeTimeout:
		if e.Retryable {
...
// GetSandboxError extracts a SandboxError from an error chain.
