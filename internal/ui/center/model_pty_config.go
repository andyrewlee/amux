...
const (
	...
	// Active tab catch-up should drain backlog quickly without creating long
	// main-thread stalls under CPU contention.
	ptyFlushChunkSizeActive = 64 * 1024
	// Flush handlers stop after this per-tick budget and reschedule immediately.
	// This keeps p95/p99 frame latency stable under heavy PTY throughput.
	ptyFlushProcessBudgetInactive = 1 * time.Millisecond
	ptyFlushProcessBudgetActive   = 1 * time.Millisecond
