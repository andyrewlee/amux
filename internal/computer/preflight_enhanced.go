...
type PreflightCheck struct {
	Name        string
	Description string
	Required    bool // If true, failure blocks computer creation
...
func RunEnhancedPreflight(ctx context.Context, providerName string, verbose bool) (*PreflightReport, error) {
	if providerName != "" && providerName != DefaultProviderName {
		return nil, fmt.Errorf("provider %q is not supported yet", providerName)
	}
	start := time.Now()
...
func checkNodeAvailable(ctx context.Context) PreflightResult {
	...
	if nodeErr != nil && npmErr != nil {
		...
		result.Suggestion = "Node.js is optional - agents will be installed in the computer"
		result.Duration = time.Since(start)
		return result
	}
...
func QuickPreflight() error {
	...
	if os.Getenv("DAYTONA_API_KEY") == "" {
		return NewComputerError(ErrCodePreflight, "api_key", nil).
			WithSuggestion("Run `amux setup` to configure your Daytona API key")
	}

	// Check SSH
	if _, err := exec.LookPath("ssh"); err != nil {
		return NewComputerError(ErrCodePreflight, "ssh", err).
	...
	if _, err := client.Do(req); err != nil {
		return NewComputerError(ErrCodeNetwork, "connectivity", err).
