...
func TestComputerError(t *testing.T) {
	t.Run("basic error creation", func(t *testing.T) {
		err := NewComputerError(ErrCodeSSH, "connect", nil)
	...
	t.Run("error with cause", func(t *testing.T) {
		...
		err := NewComputerError(ErrCodeNetwork, "dial", cause)
		if err.Cause != cause {
	...
	t.Run("error with suggestion", func(t *testing.T) {
		err := NewComputerError(ErrCodeCredentials, "setup", nil).
	...
	t.Run("retryable error", func(t *testing.T) {
		err := NewComputerError(ErrCodeSSH, "connect", nil).WithRetryable(true)
		if !err.Retryable {
	...
	t.Run("error message formatting", func(t *testing.T) {
		err := NewComputerError(ErrCodeSync, "upload", errors.New("timeout"))
		msg := err.Error()
		if msg == "" {
...
func TestErrorCodes(t *testing.T) {
	...
	for _, code := range codes {
		err := NewComputerError(code, "test", nil)
		if err.Code != code {
...
func TestIsRetryable(t *testing.T) {
	t.Run("retryable computer error", func(t *testing.T) {
		err := NewComputerError(ErrCodeSSH, "connect", nil).WithRetryable(true)
		if !IsRetryable(err) {
	...
	t.Run("non-retryable computer error", func(t *testing.T) {
		err := NewComputerError(ErrCodeConfig, "load", nil)
		if IsRetryable(err) {
			t.Error("Expected IsRetryable to return false")
		}
	})

	t.Run("non-computer error", func(t *testing.T) {
		err := errors.New("random error")
		if IsRetryable(err) {
			t.Error("Expected IsRetryable to return false for non-computer errors")
		}
	})
