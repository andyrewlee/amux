...
func GetRemoteManifest(sandbox RemoteComputer, remotePath string) (*WorkspaceManifest, error) {
	manifestPath := filepath.Join(remotePath, manifestFileName)

	// Try to read the manifest file
	resp, err := execCommand(sandbox, SafeCommands.Cat(manifestPath), nil)
...
func SaveRemoteManifest(sandbox RemoteComputer, remotePath string, manifest *WorkspaceManifest) error {
	...
	manifestPath := filepath.Join(remotePath, manifestFileName)
	return uploadBytes(ctx, sandbox, data, manifestPath)
}
...
func IncrementalSync(sandbox RemoteComputer, opts SyncOptions, verbose bool) error {
	...
	remotePath := GetWorktreeRepoPath(sandbox, opts)
	...
	_, _ = execCommand(sandbox, SafeCommands.MkdirP(remotePath), nil)

	// Delete removed files
	for _, path := range diff.Deleted {
		fullPath := filepath.Join(remotePath, path)
		logger.Debug("deleting", "path", path)
		_, _ = execCommand(sandbox, SafeCommands.RmRf(fullPath), nil)
	}
	...
	for _, path := range filesToUpload {
		...
		_, _ = execCommand(sandbox, SafeCommands.MkdirP(parentDir), nil)
		...
		uploadErr := uploadBytes(uploadCtx, sandbox, data, remoteFull)
		uploadCancel()
		if uploadErr != nil {
...
func ShouldUseIncrementalSync(sandbox RemoteComputer, opts SyncOptions) bool {
	remotePath := GetWorktreeRepoPath(sandbox, opts)
	manifestPath := filepath.Join(remotePath, manifestFileName)

	// Check if manifest exists
	resp, err := execCommand(sandbox, SafeCommands.Test("-f", manifestPath), nil)
	if err != nil || resp.ExitCode != 0 {
	...
	resp, err = execCommand(sandbox, cmd, nil)
	if err != nil || resp.ExitCode != 0 {
...
func SmartSync(sandbox RemoteComputer, opts SyncOptions, verbose bool) error {
