...
// HealthStatus represents the overall health of a computer.
...
// ComputerHealth provides health checking and self-healing capabilities.
...
// NewComputerHealth creates a new health checker for a computer.
func NewComputerHealth(client *daytona.Daytona, sandbox RemoteComputer, agent Agent) (*ComputerHealth, error) {
	daytonaSandbox, ok := sandbox.(*daytonaComputer)
	if !ok {
		return nil, fmt.Errorf("computer provider does not support Daytona health checks")
	}
...
func (h *ComputerHealth) SetVerbose(verbose bool) {
	h.verbose = verbose
}

func (h *ComputerHealth) sandboxHandle() RemoteComputer {
	return &daytonaComputer{inner: h.sandbox}
}

// Check performs all health checks and returns a report.
func (h *ComputerHealth) Check(ctx context.Context) *HealthReport {
	start := time.Now()
	report := &HealthReport{
...
// checkComputerState verifies the computer is running.
func (h *ComputerHealth) checkComputerState(ctx context.Context) HealthCheck {
	start := time.Now()
	check := HealthCheck{
		Name:    "computer_state",
...
func (h *ComputerHealth) checkCredentialDirs(ctx context.Context) HealthCheck {
	start := time.Now()
	check := HealthCheck{
...
func (h *ComputerHealth) checkAgentInstalled(ctx context.Context) HealthCheck {
	start := time.Now()
	check := HealthCheck{
...
func (h *ComputerHealth) checkNetworkConnectivity(ctx context.Context) HealthCheck {
	start := time.Now()
	check := HealthCheck{
		Name:    "network",
		Details: make(map[string]string),
...
func (h *ComputerHealth) checkDiskSpace(ctx context.Context) HealthCheck {
	start := time.Now()
	check := HealthCheck{
		Name:    "disk_space",
		Details: make(map[string]string),
...
func (h *ComputerHealth) checkProcesses(ctx context.Context) HealthCheck {
	start := time.Now()
	check := HealthCheck{
		Name:    "processes",
		Details: make(map[string]string),
...
func (h *ComputerHealth) Repair(ctx context.Context) error {
	report := h.Check(ctx)
...
func (h *ComputerHealth) repairCheck(ctx context.Context, check HealthCheck) error {
	switch check.Name {
	case "computer_state":
		// Try to restart the computer
		if h.verbose {
			fmt.Println("Attempting to restart computer...")
		}
