#!/usr/bin/env bash
# wait-for-idle.sh — compatibility wrapper for `amux assistant wait-for-idle`.
...
run_native_from_amux() {
  exec "${AMUX_BIN:-amux}" assistant wait-for-idle "$@"
}

run_native_from_checkout() {
  amux_run_from_checkout "$REPO_ROOT" "$ORIG_PWD" "assistant wait-for-idle" \
    -- "$@"
}

run_native() {
  if [[ -n "${AMUX_ASSISTANT_NATIVE_BIN:-}" ]]; then
    exec "$AMUX_ASSISTANT_NATIVE_BIN" --cwd "$ORIG_PWD" assistant wait-for-idle "$@"
  fi
