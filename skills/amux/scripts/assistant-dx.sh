#!/usr/bin/env bash
# assistant-dx.sh — Assistant-first control plane for amux coding workflows.
#
...
usage() {
  cat >&2 <<'USAGE'
Usage:
  assistant-dx.sh project add [--path <repo> | --cwd] [--workspace <name>] [--assistant <name>] [--base <branch>]
  assistant-dx.sh project list [--limit <n>] [--page <n>] [--query <text>]
  assistant-dx.sh project pick [--index <n> | --name <query> | --path <repo>] [--workspace <name>] [--assistant <name>] [--base <branch>]

  assistant-dx.sh workspace create --name <name> [--project <repo>] [--from-workspace <id>] [--scope project|nested] [--assistant <name>] [--base <branch>]
  assistant-dx.sh workspace list [--project <repo> | --all] [--workspace <id>] [--limit <n>] [--page <n>]
  assistant-dx.sh workspace decide [--project <repo>] [--from-workspace <id>] [--task <text>] [--assistant <name>] [--name <workspace-name>]

  assistant-dx.sh start --workspace <id> --prompt <text> [--assistant <name>] [--max-steps <n>] [--turn-budget <sec>] [--wait-timeout <dur>] [--idle-threshold <dur>]
  assistant-dx.sh continue [--agent <id> | --workspace <id>] [--text <text>] [--enter] [--auto-start] [--assistant <name>] [--max-steps <n>] [--turn-budget <sec>] [--wait-timeout <dur>] [--idle-threshold <dur>]

  assistant-dx.sh status [--project <repo>] [--workspace <id>] [--limit <n>] [--capture-lines <n>] [--capture-agents <n>] [--older-than <dur>] [--alerts-only] [--include-stale] [--recent-workspaces <n>]
  assistant-dx.sh alerts [same flags as status]

  assistant-dx.sh terminal run --workspace <id> --text <command> [--enter]
  assistant-dx.sh terminal preset --workspace <id> [--kind nextjs] [--port <n>] [--host <name>] [--manager auto|npm|pnpm|yarn|bun]
  assistant-dx.sh terminal logs --workspace <id> [--lines <n>]

  assistant-dx.sh cleanup [--older-than <dur>] [--yes]
  assistant-dx.sh review --workspace <id> [--assistant <name>] [--prompt <text>] [--max-steps <n>] [--turn-budget <sec>] [--wait-timeout <dur>] [--idle-threshold <dur>]
  assistant-dx.sh git ship --workspace <id> [--message <msg>] [--push]
  assistant-dx.sh guide [--project <repo>] [--workspace <id>] [--task <text>] [--assistant <name>]

  assistant-dx.sh workflow kickoff --name <workspace-name> [--project <repo>] [--from-workspace <id>] [--scope project|nested] [--assistant <name>] --prompt <text> [--base <branch>] [--max-steps <n>] [--turn-budget <sec>] [--wait-timeout <dur>] [--idle-threshold <dur>]
  assistant-dx.sh workflow dual --workspace <id> [--implement-assistant <name>] [--implement-prompt <text>] [--review-assistant <name>] [--review-prompt <text>] [--max-steps <n>] [--turn-budget <sec>] [--wait-timeout <dur>] [--idle-threshold <dur>] [--auto-continue-impl <true|false>] [--auto-continue-impl-prompt <text>]

  assistant-dx.sh assistants [--workspace <id> --probe] [--limit <n>] [--prompt <text>] [--max-steps <n>] [--turn-budget <sec>] [--wait-timeout <dur>] [--idle-threshold <dur>]
...
if AMUX_ERROR_CAPTURE_FILE="$(mktemp "${TMPDIR:-/tmp}/amux-assistant-dx-error.XXXXXX" 2>/dev/null)"; then
  :
else
  AMUX_ERROR_CAPTURE_FILE="${TMPDIR:-/tmp}/amux-assistant-dx-error.$$"
fi
_assistant_dx_cleanup() {
  if [[ -n "${AMUX_ERROR_CAPTURE_FILE:-}" && -f "$AMUX_ERROR_CAPTURE_FILE" ]]; then
    rm -f "$AMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true
  fi
}
trap _assistant_dx_cleanup EXIT
amux_ok_json() {
  local out
  AMUX_ERROR_OUTPUT=""
...
AMUX_ASSISTANT_DX_CHUNK_CHARS="${AMUX_ASSISTANT_DX_CHUNK_CHARS:-1200}"
if ! is_positive_int "$AMUX_ASSISTANT_DX_CHUNK_CHARS"; then
  AMUX_ASSISTANT_DX_CHUNK_CHARS=1200
fi

INLINE_BUTTONS_SCOPE="$(normalize_inline_buttons_scope "${AMUX_ASSISTANT_INLINE_BUTTONS_SCOPE:-allowlist}")"
INLINE_BUTTONS_ENABLED=true
if [[ "$INLINE_BUTTONS_SCOPE" == "off" ]]; then
  INLINE_BUTTONS_ENABLED=false
fi

DX_CMD_REF="skills/amux/scripts/assistant-dx.sh"
TURN_CMD_REF="skills/amux/scripts/assistant-turn.sh"
STEP_CMD_REF="skills/amux/scripts/assistant-step.sh"

normalize_command_refs() {
  local value="$1"
  value="${value//skills\/amux\/scripts\/assistant-dx.sh/$DX_CMD_REF}"
  value="${value//skills\/amux\/scripts\/assistant-turn.sh/$TURN_CMD_REF}"
  value="${value//skills\/amux\/scripts\/assistant-step.sh/$STEP_CMD_REF}"
  printf '%s' "$value"
}
...
emit_result() {
  ...
  quick_actions_json="$(jq -c --arg dx "$DX_CMD_REF" --arg turn "$TURN_CMD_REF" --arg step "$STEP_CMD_REF" '
    map(
      .command = ((.command // "")
        | gsub("skills/amux/scripts/assistant-dx\\.sh"; $dx)
        | gsub("skills/amux/scripts/assistant-turn\\.sh"; $turn)
        | gsub("skills/amux/scripts/assistant-step\\.sh"; $step)
      )
    )
  ' <<<"$quick_actions_json")"
  ...
  result_payload="$(jq -n \
    ...
    --argjson channel_chunk_chars "$AMUX_ASSISTANT_DX_CHUNK_CHARS" \
  ...
  if [[ "${AMUX_ASSISTANT_DX_SKIP_PRESENT:-false}" != "true" && -x "$AMUX_ASSISTANT_PRESENT_SCRIPT" ]]; then
    "$AMUX_ASSISTANT_PRESENT_SCRIPT" <<<"$result_payload"
  else
    printf '%s\n' "$result_payload"
  fi
}
...
emit_error() {
  ...
  RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh guide"
  RESULT_MESSAGE+=$'\n'"Next: $RESULT_NEXT_ACTION"
  RESULT_DATA="$(jq -cn --arg detail "$detail" '{error: $detail}')"
  actions='[]'
  actions="$(append_action "$actions" "guide" "Guide" "skills/amux/scripts/assistant-dx.sh guide" "primary" "Show command guidance")"
  if [[ -n "${workspace_hint// }" ]]; then
    actions="$(append_action "$actions" "status_ws" "WS Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace_hint")" "primary" "Check active workspace status")"
  else
...
emit_amux_error() {
  ...
  fallback_suggested="skills/amux/scripts/assistant-dx.sh guide"
  if [[ -n "${workspace_hint// }" ]]; then
    fallback_suggested="skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace_hint")"
  fi
  if [[ "$command_name" == terminal.* && -n "${workspace_hint// }" ]]; then
    terminal_bootstrap_cmd="skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace_hint") --text \"pwd\" --enter"
    fallback_suggested="skills/amux/scripts/assistant-dx.sh terminal logs --workspace $(shell_quote "$workspace_hint") --lines 120"
  fi
  ...
  if [[ -n "${workspace_hint// }" ]]; then
    ...
    if [[ "$command_name" == terminal.* ]]; then
      if [[ "$terminal_session_bootstrap" == "true" && -n "${terminal_bootstrap_cmd// }" ]]; then
        ...
        actions="$(append_action "$actions" "logs_ws" "WS Logs" "skills/amux/scripts/assistant-dx.sh terminal logs --workspace $(shell_quote "$workspace_hint") --lines 120" "secondary" "Inspect terminal logs for this workspace")"
      else
        actions="$(append_action "$actions" "logs_ws" "WS Logs" "skills/amux/scripts/assistant-dx.sh terminal logs --workspace $(shell_quote "$workspace_hint") --lines 120" "primary" "Inspect terminal logs for this workspace")"
      fi
    fi
  else
    actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "Show global coding status")"
...
agent_for_workspace() {
  ...
  local capture_limit
  capture_limit="${AMUX_ASSISTANT_DX_AGENT_PICK_CAPTURE_LIMIT:-4}"
...
context_file_path() {
  local configured="${AMUX_ASSISTANT_DX_CONTEXT_FILE:-}"
  ...
  printf '%s' "$base/amux/assistant-dx-context.json"
}
...
context_payload_json() {
  ...
  if [[ "${AMUX_ASSISTANT_DX_EMBED_FULL_CONTEXT:-false}" == "true" ]]; then
...
run_self_json() {
  ...
  out="$(AMUX_ASSISTANT_DX_SKIP_PRESENT=true "$SELF_SCRIPT" "$@" 2>/dev/null || true)"
  if ! jq -e . >/dev/null 2>&1 <<<"$out"; then
    return 1
  fi
  printf '%s' "$out"
...
recover_timeout_turn_once() {
  ...
  local step_script="${AMUX_ASSISTANT_DX_STEP_SCRIPT:-$SCRIPT_DIR/assistant-step.sh}"

  if [[ "${AMUX_ASSISTANT_DX_TIMEOUT_RECOVERY:-true}" == "false" ]]; then
    printf '%s' "$turn_json"
    return
  fi
  ...
  recovery_text="${AMUX_ASSISTANT_DX_TIMEOUT_RECOVERY_TEXT:-Continue from current state and provide a one-line status update plus files changed.}"
  recovery_wait="${AMUX_ASSISTANT_DX_TIMEOUT_RECOVERY_WAIT_TIMEOUT:-$wait_timeout}"
  recovery_idle="${AMUX_ASSISTANT_DX_TIMEOUT_RECOVERY_IDLE_THRESHOLD:-$idle_threshold}"

  local follow_json
  follow_json="$(AMUX_ASSISTANT_STEP_SKIP_PRESENT=true "$step_script" send \
    --agent "$agent_id" \
    --text "$recovery_text" \
    --enter \
...
normalize_turn_wait_timeout() {
  ...
  local min_seconds="${AMUX_ASSISTANT_DX_MIN_WAIT_TIMEOUT_SECONDS:-45}"
...
emit_turn_passthrough() {
  ...
  normalized_json="$(jq -c --arg dx "$DX_CMD_REF" --arg turn "$TURN_CMD_REF" --arg step "$STEP_CMD_REF" '
    def rewrite_followup_command:
      if test("assistant-(step|turn)\\.sh send --agent [^ ]+") then
        (capture("assistant-(?:step|turn)\\.sh send --agent (?<agent>[^ ]+)").agent) as $agent
        | (
            if test("--text \"[^\"]*\"") then
        ...
        | ("skills/amux/scripts/assistant-dx.sh continue --agent " + $agent + " --text " + ($text | @sh)
  ...
  if [[ "${AMUX_ASSISTANT_DX_SKIP_PRESENT:-false}" != "true" && -x "$AMUX_ASSISTANT_PRESENT_SCRIPT" ]]; then
    "$AMUX_ASSISTANT_PRESENT_SCRIPT" <<<"$normalized_json"
  else
    printf '%s\n' "$normalized_json"
  fi
}
...
cmd_project_add() {
  ...
  if [[ -n "$workspace_name" ]]; then
    ...
    if ! ws_create_out="$(amux_ok_json "${ws_args[@]}")"; then
      ...
      if workspace_create_needs_initial_commit "$err_code" "$err_message"; then
        retry_cmd="skills/amux/scripts/assistant-dx.sh project add --path $(shell_quote "$project_path") --workspace $(shell_quote "$workspace_name") --assistant $(shell_quote "${assistant:-codex}")"
        if [[ -n "$base" ]]; then
  ...
  RESULT_SUGGESTED_COMMAND=""
  if [[ -n "$workspace_id" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace_id") --assistant $(shell_quote "${assistant:-codex}") --prompt \"Analyze the biggest tech-debt items and fix the top one.\""
  else
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh workspace create --name mobile --project $(shell_quote "$project_path") --assistant codex"
  fi

  local actions='[]'
  actions="$(append_action "$actions" "ws_list" "Workspaces" "skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$project_path")" "primary" "List workspaces for this project")"
  if [[ -z "$workspace_id" ]]; then
    actions="$(append_action "$actions" "ws_create" "Create WS" "skills/amux/scripts/assistant-dx.sh workspace create --name mobile --project $(shell_quote "$project_path") --assistant codex" "success" "Create a workspace for mobile coding")"
  else
    actions="$(append_action "$actions" "start" "Start" "skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace_id") --assistant $(shell_quote "${assistant:-codex}") --prompt \"Analyze technical debt and implement the highest-impact fix.\"" "success" "Start a coding turn in this workspace")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "Show global coding status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_project_list() {
  ...
  if [[ "$count" -gt 0 ]]; then
    if [[ -n "${preferred_project_path// }" ]]; then
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project pick --path $(shell_quote "$preferred_project_path")"
    elif [[ -n "$preferred_project_name" ]]; then
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project pick --name $(shell_quote "$preferred_project_name")"
    else
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project pick --index 1"
    fi
  elif [[ -n "${query// }" ]]; then
  ...
  if [[ -n "${preferred_project_path// }" && "$preferred_project_path" != "$first_project_path" ]]; then
    actions="$(append_action "$actions" "pick_active" "Pick Active" "skills/amux/scripts/assistant-dx.sh project pick --path $(shell_quote "$preferred_project_path")" "success" "Select the active context project")"
  fi
  if [[ -n "$first_project_name" ]]; then
    actions="$(append_action "$actions" "pick1" "Pick #1" "skills/amux/scripts/assistant-dx.sh project pick --name $(shell_quote "$first_project_name")" "primary" "Select the first project")"
  fi
  if [[ -n "$first_project_path" ]]; then
    actions="$(append_action "$actions" "ws1" "WS #1" "skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$first_project_path")" "primary" "List workspaces for project #1")"
  fi
  local page_cmd_base="skills/amux/scripts/assistant-dx.sh project list --limit $limit"
  if [[ -n "${query// }" ]]; then
  ...
  if [[ -n "${query// }" ]]; then
    actions="$(append_action "$actions" "clear_query" "Clear Query" "skills/amux/scripts/assistant-dx.sh project list" "primary" "Show all projects")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "Show global coding status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_project_pick() {
  ...
  if [[ -n "${name_query// }" ]]; then
    ...
    if [[ "$match_count" -gt 1 ]]; then
      ...
      if [[ -n "${disambiguated// }" && "${disambiguated}" != "null" ]]; then
        ...
        RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project pick --index $suggested_index"
        ...
        if [[ -n "${context_project_candidate_path// }" ]]; then
          RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project pick --path $(shell_quote "$context_project_candidate_path")"
        fi
        ...
        while IFS= read -r row; do
          ...
          actions="$(append_action "$actions" "pick_${row_index}" "Pick #$row_index" "skills/amux/scripts/assistant-dx.sh project pick --index $row_index" "primary" "Select $row_name")"
        done < <(jq -c '.[0:6][]' <<<"$matches")
        actions="$(append_action "$actions" "list" "List" "skills/amux/scripts/assistant-dx.sh project list --query $(shell_quote "$name_query")" "primary" "Show filtered projects again")"
        RESULT_QUICK_ACTIONS="$actions"
  ...
  if [[ -z "$workspace_name" ]]; then
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh workspace create --name mobile --project $(shell_quote "$selected_path") --assistant codex"
    ...
    actions="$(append_action "$actions" "ws_list" "Workspaces" "skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$selected_path")" "primary" "List workspaces in this project")"
    RESULT_QUICK_ACTIONS="$actions"
  ...
  RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$ws_id") --assistant $(shell_quote "${assistant:-codex}") --prompt \"Analyze the biggest debt items and fix one high-impact issue.\""
...
cmd_guide() {
  ...
  local workspace=""
  local task=""
  local assistant="${AMUX_ASSISTANT_DX_GUIDE_ASSISTANT:-codex}"
  ...
  local capture_lines="${AMUX_ASSISTANT_DX_GUIDE_CAPTURE_LINES:-120}"
  ...
  if [[ -z "$selected_project_path" ]]; then
    if [[ "$project_count" -eq 0 ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project add --cwd --workspace mobile --assistant $(shell_quote "$assistant")"
    else
      ...
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh project list"
    fi
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh workspace decide --project $(shell_quote "$selected_project_path") --task $(shell_quote "$kickoff_prompt") --assistant $(shell_quote "$assistant")"
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$selected_project_path")"
  ...
  case "$stage" in
    add_project)
      actions="$(append_action "$actions" "add_cwd" "Add Project" "skills/amux/scripts/assistant-dx.sh project add --cwd --workspace mobile --assistant $(shell_quote "$assistant")" "success" "Register current directory and create a workspace")"
      actions="$(append_action "$actions" "project_list" "Projects" "skills/amux/scripts/assistant-dx.sh project list" "primary" "List registered projects")"
      ;;
    select_project)
      actions="$(append_action "$actions" "project_list" "Projects" "skills/amux/scripts/assistant-dx.sh project list" "primary" "List registered projects")"
      if [[ -n "$first_project_name" ]]; then
        actions="$(append_action "$actions" "pick_first" "Pick #1" "skills/amux/scripts/assistant-dx.sh project pick --name $(shell_quote "$first_project_name")" "success" "Select the first listed project")"
      fi
      ;;
    create_workspace)
      actions="$(append_action "$actions" "decide_ws" "Decide WS" "skills/amux/scripts/assistant-dx.sh workspace decide --project $(shell_quote "$selected_project_path") --task $(shell_quote "$kickoff_prompt") --assistant $(shell_quote "$assistant")" "success" "Get a project workspace vs nested workspace recommendation")"
      actions="$(append_action "$actions" "create_ws" "Create WS" "skills/amux/scripts/assistant-dx.sh workspace create --name mobile --project $(shell_quote "$selected_project_path") --assistant $(shell_quote "$assistant")" "primary" "Create a project workspace")"
      ;;
    select_workspace)
      actions="$(append_action "$actions" "list_ws" "Workspaces" "skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$selected_project_path")" "primary" "List workspaces in this project")"
      if [[ -n "$first_workspace_id" ]]; then
        actions="$(append_action "$actions" "start_ws" "Start #1" "skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$first_workspace_id") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")" "success" "Start coding in the first workspace")"
      fi
      ;;
    start_agent)
      actions="$(append_action "$actions" "start" "Start" "skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$selected_workspace_id") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")" "success" "Start coding turn")"
      actions="$(append_action "$actions" "dual" "Dual Pass" "skills/amux/scripts/assistant-dx.sh workflow dual --workspace $(shell_quote "$selected_workspace_id") --implement-assistant claude --review-assistant codex" "primary" "Implement then review with separate assistants")"
    ...
    restart_agent)
      ...
      ;;
    review_and_ship)
      actions="$(append_action "$actions" "review" "Review" "skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$selected_workspace_id") --assistant codex" "success" "Review uncommitted changes")"
      actions="$(append_action "$actions" "ship" "Ship" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$selected_workspace_id")" "primary" "Commit current changes")"
      actions="$(append_action "$actions" "dual" "Dual Pass" "skills/amux/scripts/assistant-dx.sh workflow dual --workspace $(shell_quote "$selected_workspace_id") --implement-assistant claude --review-assistant codex" "primary" "Run implementation+review pass")"
      ;;
    continue_agent)
      actions="$(append_action "$actions" "continue" "Continue" "$RESULT_SUGGESTED_COMMAND" "success" "Continue active coding turn")"
      actions="$(append_action "$actions" "status_ws" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$selected_workspace_id")" "primary" "Check workspace state")"
      actions="$(append_action "$actions" "logs" "Terminal Logs" "skills/amux/scripts/assistant-dx.sh terminal logs --workspace $(shell_quote "$selected_workspace_id") --lines 120" "primary" "Inspect terminal output")"
      ;;
  esac
...
workspace_create_emit_existing_recovery() {
  ...
  RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$existing_id") --assistant $(shell_quote "$suggested_assistant") --prompt \"Summarize current status and continue with next high-impact task.\""
  ...
  actions="$(append_action "$actions" "list_ws" "Workspaces" "skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$project")" "primary" "List all workspaces for this project")"
  actions="$(append_action "$actions" "retry_new_name" "New Name" "skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$alt_name") --project $(shell_quote "$project") --scope $(shell_quote "$requested_scope") --assistant $(shell_quote "$suggested_assistant")" "primary" "Retry workspace creation with a new name")"
  RESULT_QUICK_ACTIONS="$actions"
...
emit_project_add_workspace_conflict_guidance() {
  ...
  retry_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$alt_name") --project $(shell_quote "$project") --scope project --assistant $(shell_quote "$suggested_assistant")"
  if [[ -n "$requested_base" ]]; then
    retry_cmd+=" --base $(shell_quote "$requested_base")"
  fi
  list_cmd="skills/amux/scripts/assistant-dx.sh workspace list --project $(shell_quote "$project")"
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --project $(shell_quote "$project")" "primary" "Check project status before retrying")"
  RESULT_QUICK_ACTIONS="$actions"
...
emit_initial_commit_guidance() {
  ...
  actions="$(append_action "$actions" "project_only" "Project Only" "skills/amux/scripts/assistant-dx.sh project add --path $(shell_quote "$project")" "primary" "Register project without creating a workspace")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workspace_create() {
  ...
  if [[ -n "$from_workspace" && -n "$project_for_match" && -n "$parent_repo_for_match" && "$project_for_match" != "$parent_repo_for_match" ]]; then
    ...
    retry_without_project="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$name") --from-workspace $(shell_quote "$from_workspace") --scope $(shell_quote "$scope")"
    retry_with_parent_repo="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$name") --from-workspace $(shell_quote "$from_workspace") --project $(shell_quote "$parent_repo") --scope $(shell_quote "$scope")"
    if [[ -n "$assistant" ]]; then
  ...
  if ! out="$(amux_ok_json "${args[@]}")"; then
    ...
    if workspace_create_needs_initial_commit "$err_code" "$err_message"; then
      local retry_cmd
      retry_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$name") --project $(shell_quote "$project") --scope $(shell_quote "$scope") --assistant $(shell_quote "${assistant:-codex}")"
      if [[ -n "$from_workspace" ]]; then
        retry_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$name") --from-workspace $(shell_quote "$from_workspace") --scope $(shell_quote "$scope") --assistant $(shell_quote "${assistant:-codex}")"
      fi
      if [[ -n "$base" ]]; then
        retry_cmd+=" --base $(shell_quote "$base")"
      fi
    ...
    if [[ -z "${out// }" ]] || ! jq -e '.ok == true' >/dev/null 2>&1 <<<"$out"; then
      if [[ "$err_code" == "create_failed" ]] && [[ "$err_message" == *"already exists"* || "$err_message" == *"already used by worktree"* ]]; then
        ...
        retry_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$alt_name") --project $(shell_quote "$project") --scope $(shell_quote "$scope") --assistant $(shell_quote "${assistant:-codex}")"
        if [[ -n "$from_workspace" ]]; then
          retry_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$alt_name") --from-workspace $(shell_quote "$from_workspace") --scope $(shell_quote "$scope") --assistant $(shell_quote "${assistant:-codex}")"
        fi
        if [[ -n "$base" ]]; then
          retry_cmd+=" --base $(shell_quote "$base")"
        fi
  ...
  RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$ws_id") --assistant $(shell_quote "${assistant_out:-codex}") --prompt \"Analyze the biggest debt item and implement the fix.\""

  local actions='[]'
  actions="$(append_action "$actions" "start" "Start" "$RESULT_SUGGESTED_COMMAND" "success" "Start a coding turn in this workspace")"
  actions="$(append_action "$actions" "term" "Terminal" "skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$ws_id") --text \"pwd\" --enter" "primary" "Run a terminal command in this workspace")"
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$ws_id")" "primary" "Show workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workspace_list() {
  ...
  if [[ -n "$first_ws" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$first_ws") --assistant codex --prompt \"Summarize current objectives and pick the next coding task.\""
  fi
  ...
  if [[ -n "$first_ws" ]]; then
    ...
    actions="$(append_action "$actions" "status" "Status #1" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$first_ws")" "primary" "Check status for the first listed workspace")"
  fi
  local list_cmd_base="skills/amux/scripts/assistant-dx.sh workspace list --limit $limit"
  ...
  if [[ "$project_from_context" == "true" ]]; then
    actions="$(append_action "$actions" "all_projects" "All Projects" "skills/amux/scripts/assistant-dx.sh workspace list --all --limit $(shell_quote "$limit")" "primary" "List workspaces across all projects")"
  fi
  actions="$(append_action "$actions" "global" "Global" "skills/amux/scripts/assistant-dx.sh status" "primary" "Show global coding status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workspace_decide() {
  ...
  local from_workspace=""
  local task=""
  local assistant="${AMUX_ASSISTANT_DX_DECIDE_ASSISTANT:-codex}"
  local name=""
  ...
  project_create_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$final_project_name") --project $(shell_quote "$project") --assistant $(shell_quote "$assistant")"
  if [[ -n "$parent_id" ]]; then
    nested_create_cmd="skills/amux/scripts/assistant-dx.sh workspace create --name $(shell_quote "$name") --from-workspace $(shell_quote "$parent_id") --scope nested --assistant $(shell_quote "$assistant")"
  else
    nested_create_cmd=""
  fi
  ...
  if [[ "$recommendation" == "nested" && -n "$parent_id" ]]; then
    recommended_command="skills/amux/scripts/assistant-dx.sh workflow kickoff --from-workspace $(shell_quote "$parent_id") --scope nested --name $(shell_quote "$name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    alternate_command="skills/amux/scripts/assistant-dx.sh workflow kickoff --project $(shell_quote "$project") --name $(shell_quote "$final_project_name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
  else
    recommended_command="skills/amux/scripts/assistant-dx.sh workflow kickoff --project $(shell_quote "$project") --name $(shell_quote "$final_project_name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    if [[ -n "$parent_id" ]]; then
      alternate_command="skills/amux/scripts/assistant-dx.sh workflow kickoff --from-workspace $(shell_quote "$parent_id") --scope nested --name $(shell_quote "$name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    fi
  fi
...
cmd_start() {
  ...
  start_command_error_retry="${AMUX_ASSISTANT_DX_START_COMMAND_ERROR_RETRY:-true}"
  if [[ "$start_command_error_retry" != "false" ]] && jq -e . >/dev/null 2>&1 <<<"$turn_json"; then
    ...
    if [[ "$start_retry_status" == "command_error" || "$start_retry_status" == "partial" ]] \
      ...
      start_retry_json="$(AMUX_ASSISTANT_TURN_SKIP_PRESENT=true "$TURN_SCRIPT" run \
        --workspace "$workspace" \
        --assistant "$assistant" \
        --prompt "$prompt" \
        --max-steps "$max_steps" \
  ...
  start_prune_retry="${AMUX_ASSISTANT_DX_START_PRUNE_RETRY:-true}"
  start_prune_older_than="${AMUX_ASSISTANT_DX_START_PRUNE_OLDER_THAN:-2m}"
  if [[ "$start_prune_retry" != "false" ]] && jq -e . >/dev/null 2>&1 <<<"$turn_json"; then
    ...
    if [[ "$start_prune_status" == "command_error" || "$start_prune_status" == "partial" ]] \
      ...
      start_prune_retry_json="$(AMUX_ASSISTANT_TURN_SKIP_PRESENT=true "$TURN_SCRIPT" run \
        --workspace "$workspace" \
        --assistant "$assistant" \
        --prompt "$prompt" \
        --max-steps "$max_steps" \
  ...
  start_step_fallback="${AMUX_ASSISTANT_DX_START_STEP_FALLBACK:-true}"
  ...
  start_command_error_fallback_assistant="${AMUX_ASSISTANT_DX_START_COMMAND_ERROR_FALLBACK_ASSISTANT:-gemini}"
  if jq -e . >/dev/null 2>&1 <<<"$turn_json"; then
  ...
  nochange_retry_enabled="${AMUX_ASSISTANT_DX_NOCHANGE_RETRY:-true}"
  nochange_fallback_assistant="${AMUX_ASSISTANT_DX_NOCHANGE_FALLBACK_ASSISTANT:-codex}"
...
cmd_continue() {
    ...
    if active_agents_out="$(amux_ok_json agent list)"; then
      ...
      if [[ "$active_count" == "1" ]]; then
        ...
        if [[ -n "$first_agent" ]]; then
          RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$first_agent") --text \"Continue from current state and report status plus next action.\" --enter"
        elif [[ -n "$first_workspace" ]]; then
          RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh continue --workspace $(shell_quote "$first_workspace") --text \"Continue from current state and report status plus next action.\" --enter"
        fi
        ...
        while IFS= read -r row; do
          ...
          continue_cmd="skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$row_agent") --text \"Continue from current state and report status plus next action.\" --enter"
        ...
        actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "See all active agents and alerts")"
        if [[ -n "$preferred_project" ]]; then
          actions="$(append_action "$actions" "status_project" "Project Status" "skills/amux/scripts/assistant-dx.sh status --project $(shell_quote "$preferred_project")" "primary" "Show active agents in the current project")"
        fi
        RESULT_QUICK_ACTIONS="$actions"
  ...
  if [[ -z "$agent" && -n "$workspace" ]]; then
    ...
    if [[ -z "$agent" ]]; then
        ...
        if other_agents_out="$(amux_ok_json agent list)"; then
          ...
          if [[ "$other_active_count" =~ ^[0-9]+$ ]] && [[ "$other_active_count" -gt 0 ]]; then
            ...
            while IFS= read -r row; do
              ...
              row_workspace_label="$(workspace_brief_label "$row_workspace" "$row_workspace_name" "$row_scope_label" "$row_parent")"
              row_continue_cmd="skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$row_agent") --text \"Continue from current state and report status plus next action.\" --enter"
            ...
            other_actions="$(append_action "$other_actions" "auto_start" "Auto Start Here" "skills/amux/scripts/assistant-dx.sh continue --workspace $(shell_quote "$workspace") --auto-start --assistant $(shell_quote "$suggested_assistant") --text \"Resume work and provide status plus next action.\"" "success" "Auto-start current workspace and continue")"
            other_actions="$(append_action "$other_actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "Show all active agents and alerts")"
            ...
            if [[ -n "$first_other_agent" ]]; then
              RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$first_other_agent") --text \"Continue from current state and report status plus next action.\" --enter"
            elif [[ -n "$first_other_workspace" ]]; then
              RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh continue --workspace $(shell_quote "$first_other_workspace") --text \"Continue from current state and report status plus next action.\" --enter"
            else
              RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh status"
            fi
      ...
      auto_start_cmd="skills/amux/scripts/assistant-dx.sh continue --workspace $(shell_quote "$workspace") --auto-start --assistant $(shell_quote "$suggested_assistant") --text \"Resume work and provide status plus next action.\""
...
cmd_status() {
  local result_command="${AMUX_ASSISTANT_DX_STATUS_RESULT_COMMAND:-status}"
  ...
  local workspace=""
  local limit=12
  local capture_lines="${AMUX_ASSISTANT_DX_STATUS_CAPTURE_LINES:-120}"
  local capture_agents_default="${AMUX_ASSISTANT_DX_STATUS_CAPTURE_AGENTS:-6}"
  local capture_agents="$capture_agents_default"
  local capture_agents_explicit=false
  local older_than="${AMUX_ASSISTANT_DX_STATUS_ALERT_OLDER_THAN:-24h}"
  local recent_workspaces="${AMUX_ASSISTANT_DX_STATUS_RECENT_WORKSPACES:-4}"
  ...
  if [[ "${AMUX_ASSISTANT_DX_FORCE_ALERTS_ONLY:-false}" == "true" ]]; then
    alerts_only=true
  fi
  if [[ "${AMUX_ASSISTANT_DX_STATUS_INCLUDE_STALE_ALERTS:-false}" == "true" ]]; then
  ...
  if [[ "$capture_agents_explicit" != "true" ]]; then
    if [[ -n "$workspace" ]]; then
      capture_agents="${AMUX_ASSISTANT_DX_STATUS_CAPTURE_AGENTS_WORKSPACE:-3}"
    elif [[ -n "$project" ]]; then
      capture_agents="${AMUX_ASSISTANT_DX_STATUS_CAPTURE_AGENTS_PROJECT:-2}"
    fi
  fi
  ...
  local refresh_cmd
  refresh_cmd="skills/amux/scripts/assistant-dx.sh $result_command"
  if [[ -n "$project" ]]; then
  ...
  if [[ -n "$first_needs_input_agent" ]]; then
    ...
    if [[ -n "$first_completed_workspace" ]]; then
      suggested_command="skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$first_completed_workspace") --assistant codex"
    elif [[ -n "$first_completed_agent" ]]; then
      suggested_command="skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$first_completed_agent") --text \"Summarize final changes, tests, and remaining risks in 5 bullets.\" --enter"
    fi
  elif [[ "$stale_alert_count" -gt 0 ]]; then
    next_action="Clean stale sessions to reduce noise."
    suggested_command="skills/amux/scripts/assistant-dx.sh cleanup --older-than $(shell_quote "$older_than") --yes"
  ...
  if [[ "$alerts_only" == "true" && "$status" == "ok" ]]; then
    ...
    full_status_cmd="skills/amux/scripts/assistant-dx.sh status"
    if [[ -n "$project" ]]; then
  ...
  if [[ "$alerts_only" == "true" ]]; then
    ...
    full_status_action="skills/amux/scripts/assistant-dx.sh status"
    if [[ -n "$project" ]]; then
  ...
  if [[ "$completed_count" -gt 0 ]]; then
    if [[ -n "$first_completed_workspace" ]]; then
      actions="$(append_action "$actions" "review_done" "Review Done" "skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$first_completed_workspace") --assistant codex" "success" "Review completed workspace changes")"
      actions="$(append_action "$actions" "ship_done" "Ship Done" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$first_completed_workspace") --push" "primary" "Commit and push completed workspace changes")"
    elif [[ -n "$first_completed_agent" ]]; then
      actions="$(append_action "$actions" "summary_done" "Summarize" "skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$first_completed_agent") --text \"Summarize final changes, tests, and risks.\" --enter" "primary" "Capture final summary for completed agent")"
    fi
  fi
  if [[ "$stale_alert_count" -gt 0 ]]; then
    actions="$(append_action "$actions" "cleanup" "Cleanup" "skills/amux/scripts/assistant-dx.sh cleanup --older-than $(shell_quote "$older_than") --yes" "danger" "Prune stale sessions")"
  fi
  local first_ws
  first_ws="$(jq -r '.[0].id // ""' <<<"$ws_enriched")"
  if [[ "$needs_input_count" -eq 0 && -n "$workspace" && -n "$first_active_agent" ]]; then
    actions="$(append_action "$actions" "continue_agent" "Continue Agent" "skills/amux/scripts/assistant-dx.sh continue --agent $(shell_quote "$first_active_agent") --text \"Status update and next action.\" --enter" "success" "Continue active agent in this workspace")"
...
cmd_alerts() {
  AMUX_ASSISTANT_DX_FORCE_ALERTS_ONLY=true AMUX_ASSISTANT_DX_STATUS_RESULT_COMMAND=alerts cmd_status "$@"
}
...
cmd_terminal_run() {
  ...
  RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal logs --workspace $(shell_quote "$workspace") --lines 120"
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_terminal_preset() {
  local workspace=""
  local kind="nextjs"
  local port="${AMUX_ASSISTANT_DX_TERMINAL_PORT:-3000}"
  local host="${AMUX_ASSISTANT_DX_TERMINAL_HOST:-0.0.0.0}"
  local manager="auto"
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  actions="$(append_action "$actions" "alerts" "Alerts" "skills/amux/scripts/assistant-dx.sh alerts --workspace $(shell_quote "$workspace")" "primary" "Check blockers requiring attention")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_terminal_logs() {
  local workspace=""
  local lines=200
  local retry_attempts="${AMUX_ASSISTANT_DX_TERMINAL_LOGS_RETRIES:-4}"
  local retry_delay_seconds="${AMUX_ASSISTANT_DX_TERMINAL_LOGS_RETRY_DELAY:-1}"
  ...
  while true; do
    ...
    if { [[ "$err_code" == "not_found" && "$err_message" == *"no terminal session found for workspace"* ]]; } || [[ "$err_out" == *"no terminal session found for workspace"* ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"pwd\" --enter"
      ...
      actions="$(append_action "$actions" "preset" "Preset" "skills/amux/scripts/assistant-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs" "success" "Start a Next.js dev terminal preset")"
      actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
      RESULT_QUICK_ACTIONS="$actions"
  ...
  default_suggested_command="skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"npm test\" --enter"
  ...
  if [[ "$content_lower" == *"err_ngrok_108"* || ( "$content_lower" == *"ngrok"* && "$content_lower" == *"authentication failed"* ) ]]; then
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"ngrok http 3000\" --enter"
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"pwd && ls -la\" --enter"
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs --port 3001"
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs --port 3000"
  ...
  if [[ "$diagnostic_reason" == "ngrok_auth_session_limit" ]]; then
    actions="$(append_action "$actions" "retry_ngrok" "Retry Ngrok" "skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"ngrok http 3000\" --enter" "primary" "Retry tunnel without terminating other local ngrok processes")"
    actions="$(append_action "$actions" "logs_refresh" "Refresh Logs" "skills/amux/scripts/assistant-dx.sh terminal logs --workspace $(shell_quote "$workspace") --lines $(shell_quote "$lines")" "primary" "Re-check tunnel logs after fixing auth/session limits")"
  elif [[ "$diagnostic_reason" == "missing_package_manifest" ]]; then
    actions="$(append_action "$actions" "preset_nextjs" "Preset Next.js" "skills/amux/scripts/assistant-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs" "primary" "Retry Next.js preset in the correct workspace")"
  elif [[ "$diagnostic_reason" == "port_in_use" ]]; then
    actions="$(append_action "$actions" "preset_port_3001" "Use Port 3001" "skills/amux/scripts/assistant-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs --port 3001" "primary" "Retry server startup on an alternate port")"
  elif [[ "$diagnostic_reason" == "local_server_unreachable" ]]; then
    actions="$(append_action "$actions" "start_server" "Start Server" "skills/amux/scripts/assistant-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs --port 3000" "primary" "Start a local server before tunneling")"
    actions="$(append_action "$actions" "retry_ngrok" "Retry Ngrok" "skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"ngrok http 3000\" --enter" "primary" "Retry tunnel after server is running")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_cleanup() {
  local older_than="${AMUX_ASSISTANT_DX_CLEANUP_OLDER_THAN:-24h}"
  local yes=false
  ...
  if [[ "$dry_run" == "true" && "$total" -gt 0 ]]; then
    actions="$(append_action "$actions" "confirm" "Confirm" "skills/amux/scripts/assistant-dx.sh cleanup --older-than $(shell_quote "$older_than") --yes" "danger" "Prune stale sessions now")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "Refresh global status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_git_ship() {
  ...
  if [[ -z "${porcelain// }" ]]; then
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace")"
    ...
    if [[ "$push" == "true" && "$pushed" == "true" ]]; then
      ...
      if [[ "$push_error" == "origin remote is not configured" ]]; then
        ...
        RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"git remote -v\" --enter"
      else
        RESULT_NEXT_ACTION="Fix push issues, then retry push."
        RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace") --push"
      fi
    ...
    if [[ "$push" != "true" && "$suggest_push" == "true" ]]; then
      actions="$(append_action "$actions" "push" "Push" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace") --push" "success" "Push existing commits to remote")"
    fi
    if [[ "$push" == "true" && "$push_error" == "origin remote is not configured" ]]; then
      actions="$(append_action "$actions" "show_remote" "Remote" "skills/amux/scripts/assistant-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"git remote -v\" --enter" "primary" "Inspect git remotes in this workspace")"
      actions="$(append_action "$actions" "retry_push" "Retry Push" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace") --push" "primary" "Retry push after configuring origin")"
    elif [[ "$push" == "true" && -n "$push_error" ]]; then
      actions="$(append_action "$actions" "retry_push" "Retry Push" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace") --push" "primary" "Retry push after fixing errors")"
    fi
    actions="$(append_action "$actions" "review" "Review" "skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace")" "primary" "Run review workflow")"
    actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
    RESULT_QUICK_ACTIONS="$actions"
  ...
  local actions='[]'
  if [[ "$pushed" != "true" ]]; then
    actions="$(append_action "$actions" "push" "Push" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace") --push" "success" "Push latest commit")"
  fi
  actions="$(append_action "$actions" "review" "Review" "skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace")" "primary" "Run review workflow")"
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workflow_kickoff() {
  ...
  kickoff_auto_continue="${AMUX_ASSISTANT_DX_KICKOFF_NEEDS_INPUT_AUTO_CONTINUE:-true}"
  kickoff_auto_text="${AMUX_ASSISTANT_DX_KICKOFF_NEEDS_INPUT_TEXT:-If a choice is required, pick the safest high-impact default, continue, and report status plus next action.}"
...
cmd_workflow_dual() {
  ...
  local implement_prompt="${AMUX_ASSISTANT_DX_IMPLEMENT_PROMPT:-Identify the highest-impact technical-debt item in this workspace, implement the fix, run targeted validation, and summarize changed files plus remaining risks.}"
  local review_assistant="${AMUX_ASSISTANT_DX_REVIEW_ASSISTANT:-codex}"
  local review_prompt="${AMUX_ASSISTANT_DX_REVIEW_PROMPT:-Review current uncommitted changes. Return findings first ordered by severity with file references, then residual risks and test gaps.}"
  local auto_continue_impl="${AMUX_ASSISTANT_DX_DUAL_AUTO_CONTINUE_IMPL:-true}"
  ...
  local wait_timeout="${AMUX_ASSISTANT_DX_WAIT_TIMEOUT:-60s}"
  local idle_threshold="${AMUX_ASSISTANT_DX_IDLE_THRESHOLD:-10s}"
  local progress_stderr="${AMUX_ASSISTANT_DX_PROGRESS_STDERR:-true}"
  ...
  dx_dual_progress() {
    ...
    printf '[assistant-dx][workflow dual][%ss] %s\n' "$elapsed" "$message" >&2
  }
  ...
  RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace")"
  local codex_continue_cmd
  codex_continue_cmd="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant codex --prompt \"Continue from current state and provide concise status plus next action.\""
  ...
  if [[ "$impl_ok" != "true" ]]; then
    ...
    if [[ -n "$impl_cmd" ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$implement_assistant") --prompt $(shell_quote "$implement_prompt")"
    fi
    ...
    if [[ "$implement_assistant" != "codex" ]] && { [[ -z "$impl_cmd" ]] || [[ "$impl_cmd" == *"assistant-step.sh send --agent"* ]] || [[ "$impl_cmd" == *"assistant-step.sh send --agent"* ]]; }; then
    ...
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$effective_review_assistant")"
  ...
  if [[ "$RESULT_STATUS" != "ok" ]]; then
    if [[ "$review_json" != "null" ]] && [[ "$review_status" != "completed" ]] && [[ "$review_status" != "idle" ]]; then
      if [[ -z "${RESULT_SUGGESTED_COMMAND// }" ]] || [[ "$RESULT_SUGGESTED_COMMAND" == "skills/amux/scripts/assistant-dx.sh git ship --workspace $workspace" ]]; then
        RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$effective_review_assistant")"
      fi
      ...
      if [[ -z "${RESULT_SUGGESTED_COMMAND// }" ]] || [[ "$RESULT_SUGGESTED_COMMAND" == "skills/amux/scripts/assistant-dx.sh git ship --workspace $workspace" ]]; then
        RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$effective_implement_assistant") --prompt $(shell_quote "$implement_prompt")"
      fi
    fi
    if [[ -z "${RESULT_SUGGESTED_COMMAND// }" ]]; then
      RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")"
    fi
  fi
  ...
  if [[ "$review_json" == "null" && "$review_skipped_reason" != "implementation_not_ready" ]]; then
    actions="$(append_action "$actions" "run_review" "Run Review" "skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$effective_review_assistant")" "primary" "Run review phase now")"
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  actions="$(append_action "$actions" "alerts" "Alerts" "skills/amux/scripts/assistant-dx.sh alerts --workspace $(shell_quote "$workspace")" "primary" "Check blocking alerts")"
  if [[ "$RESULT_STATUS" == "ok" ]]; then
    actions="$(append_action "$actions" "ship" "Ship" "skills/amux/scripts/assistant-dx.sh git ship --workspace $(shell_quote "$workspace")" "success" "Commit current changes")"
  fi
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_assistants() {
  ...
  local workspace=""
  local probe=false
  local limit="${AMUX_ASSISTANT_DX_ASSISTANTS_LIMIT:-6}"
  local probe_prompt="${AMUX_ASSISTANT_DX_ASSISTANTS_PROBE_PROMPT:-Reply in one line with READY and the top current objective for this workspace.}"
  local max_steps="${AMUX_ASSISTANT_DX_MAX_STEPS:-2}"
  local turn_budget="${AMUX_ASSISTANT_DX_TURN_BUDGET:-150}"
  local wait_timeout="${AMUX_ASSISTANT_DX_WAIT_TIMEOUT:-45s}"
  local idle_threshold="${AMUX_ASSISTANT_DX_IDLE_THRESHOLD:-8s}"
  ...
  if [[ -n "$workspace" && "$dual_ready" == "true" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh workflow dual --workspace $(shell_quote "$workspace") --implement-assistant claude --review-assistant codex"
  elif [[ -n "$workspace" && -n "$preferred_assistant" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$preferred_assistant") --prompt \"Summarize current status and next action in one line.\""
  elif [[ -n "$workspace" && -n "$first_ready" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$first_ready") --prompt \"Summarize current status and next action in one line.\""
  fi

  local actions='[]'
  actions="$(append_action "$actions" "status" "Status" "skills/amux/scripts/assistant-dx.sh status" "primary" "Show current work/agent status")"
  local review_cmd
  review_cmd="skills/amux/scripts/assistant-dx.sh review --workspace <workspace_id> --assistant codex"
  if [[ -n "$workspace" ]]; then
    review_cmd="skills/amux/scripts/assistant-dx.sh review --workspace $(shell_quote "$workspace") --assistant codex"
  fi
  actions="$(append_action "$actions" "review" "Review" "$review_cmd" "primary" "Run a review workflow")"
  if [[ "$probe" != "true" && -n "$workspace" ]]; then
    actions="$(append_action "$actions" "probe" "Probe" "skills/amux/scripts/assistant-dx.sh assistants --workspace $(shell_quote "$workspace") --probe --limit $(shell_quote "$limit")" "primary" "Run readiness probes for ready assistants")"
  fi
  if [[ -n "$workspace" && "$dual_ready" == "true" ]]; then
    actions="$(append_action "$actions" "dual" "Dual Pass" "skills/amux/scripts/assistant-dx.sh workflow dual --workspace $(shell_quote "$workspace") --implement-assistant claude --review-assistant codex" "success" "Implement with claude and review with codex")"
  elif [[ -n "$workspace" && -n "$preferred_assistant" ]]; then
    actions="$(append_action "$actions" "start_ready" "Start Ready" "skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$preferred_assistant") --prompt \"Summarize current status and next action in one line.\"" "primary" "Start with best probe-passed assistant")"
  elif [[ -n "$workspace" && -n "$first_ready" ]]; then
    actions="$(append_action "$actions" "start_ready" "Start Ready" "skills/amux/scripts/assistant-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$first_ready") --prompt \"Summarize current status and next action in one line.\"" "primary" "Start with first ready assistant")"
  fi
  RESULT_QUICK_ACTIONS="$actions"
...
flag_requires_value() {
  # Keep this list in sync with assistant-step.sh/assistant-turn.sh preflight validators.
  local flag="$1"
  case "$flag" in
...
flag_allows_flag_like_value() {
  # Keep this allowlist in sync with assistant-step.sh/assistant-turn.sh preflight validators.
  local flag="$1"
  case "$flag" in
...
TURN_SCRIPT="${AMUX_ASSISTANT_DX_TURN_SCRIPT:-$SCRIPT_DIR/assistant-turn.sh}"
if [[ ! -x "$TURN_SCRIPT" ]]; then
  TURN_SCRIPT="$SCRIPT_DIR/assistant-turn.sh"
fi
SELF_SCRIPT="${AMUX_ASSISTANT_DX_SELF_SCRIPT:-$SCRIPT_DIR/assistant-dx.sh}"
if [[ ! -x "$SELF_SCRIPT" ]]; then
  SELF_SCRIPT="$SCRIPT_PATH"
fi
STEP_SCRIPT_PATH="${AMUX_ASSISTANT_DX_STEP_SCRIPT:-$SCRIPT_DIR/assistant-step.sh}"
if [[ ! -x "$STEP_SCRIPT_PATH" ]]; then
  STEP_SCRIPT_PATH="$SCRIPT_DIR/assistant-step.sh"
fi
AMUX_ASSISTANT_PRESENT_SCRIPT="${AMUX_ASSISTANT_PRESENT_SCRIPT:-$SCRIPT_DIR/assistant-present.sh}"

DX_CMD_REF="${AMUX_ASSISTANT_DX_CMD_REF:-skills/amux/scripts/assistant-dx.sh}"
TURN_CMD_REF="${AMUX_ASSISTANT_DX_TURN_CMD_REF:-skills/amux/scripts/assistant-turn.sh}"
STEP_CMD_REF="${AMUX_ASSISTANT_DX_STEP_CMD_REF:-skills/amux/scripts/assistant-step.sh}"
...
case "$top_cmd" in
  ...
  help|-h|--help)
    ...
    RESULT_COMMAND="help"
    RESULT_STATUS="ok"
    RESULT_SUMMARY="assistant-dx help"
    RESULT_MESSAGE="ℹ️ assistant-dx help printed to stderr"
    RESULT_DATA='{}'
    RESULT_QUICK_ACTIONS='[]'
    emit_result
    ;;
