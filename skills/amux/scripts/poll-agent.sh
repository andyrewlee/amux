#!/usr/bin/env bash
# poll-agent.sh — compatibility wrapper for `amux assistant poll-agent`.
...
run_native_from_amux() {
  exec "${AMUX_BIN:-amux}" assistant poll-agent "$@"
}

run_native_from_checkout() {
  amux_run_from_checkout "$REPO_ROOT" "$ORIG_PWD" "assistant poll-agent" \
    -- "$@"
}

run_native() {
  if [[ -n "${AMUX_ASSISTANT_NATIVE_BIN:-}" ]]; then
    exec "$AMUX_ASSISTANT_NATIVE_BIN" --cwd "$ORIG_PWD" assistant poll-agent "$@"
  fi
