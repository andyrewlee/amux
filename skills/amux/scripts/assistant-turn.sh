...
WAIT_TIMEOUT="60s"
IDLE_THRESHOLD="10s"
MAX_STEPS="${AMUX_ASSISTANT_TURN_MAX_STEPS:-3}"
TURN_BUDGET_SECONDS="${AMUX_ASSISTANT_TURN_BUDGET_SECONDS:-180}"
FOLLOWUP_TEXT="${AMUX_ASSISTANT_TURN_FOLLOWUP_TEXT:-Continue from current state and provide a concise status update and next action.}"
TIMEOUT_STREAK_LIMIT="${AMUX_ASSISTANT_TURN_TIMEOUT_STREAK_LIMIT:-2}"
COALESCE_MILESTONES="${AMUX_ASSISTANT_TURN_COALESCE_MILESTONES:-true}"
FINAL_RESERVE_SECONDS="${AMUX_ASSISTANT_TURN_FINAL_RESERVE_SECONDS:-20}"
AMUX_ASSISTANT_CHUNK_CHARS="${AMUX_ASSISTANT_TURN_CHUNK_CHARS:-1200}"
TURN_VERBOSITY="$(normalize_verbosity_level "${AMUX_ASSISTANT_TURN_VERBOSITY:-normal}")"

SCRIPT_SOURCE="${BASH_SOURCE[0]:-$0}"
SCRIPT_DIR="$(cd "$(dirname "$SCRIPT_SOURCE")" >/dev/null 2>&1 && pwd -P)"
SCRIPT_PATH="$SCRIPT_DIR/$(basename "$SCRIPT_SOURCE")"
TURN_SCRIPT_REF="${AMUX_ASSISTANT_TURN_CMD_REF:-skills/amux/scripts/assistant-turn.sh}"
TURN_SCRIPT_CMD="$(shell_quote "$TURN_SCRIPT_REF")"
STEP_SCRIPT="${AMUX_ASSISTANT_TURN_STEP_SCRIPT:-$SCRIPT_DIR/assistant-step.sh}"
if [[ ! -x "$STEP_SCRIPT" ]]; then
  STEP_SCRIPT="$SCRIPT_DIR/assistant-step.sh"
fi
STEP_SCRIPT_REF="${AMUX_ASSISTANT_TURN_STEP_CMD_REF:-skills/amux/scripts/assistant-step.sh}"
...
if [[ ! -x "$STEP_SCRIPT" ]]; then
  echo '{"ok":false,"status":"command_error","summary":"assistant-step.sh is not executable","error":"invalid step script path"}'
  exit 0
fi
...
if ! [[ "$AMUX_ASSISTANT_CHUNK_CHARS" =~ ^[0-9]+$ ]] || [[ "$AMUX_ASSISTANT_CHUNK_CHARS" -le 0 ]]; then
  AMUX_ASSISTANT_CHUNK_CHARS=1200
fi
...
while [[ "$STEPS_USED" -lt "$MAX_STEPS" ]]; do
  ...
  if [[ "$CURRENT_MODE" == "run" ]]; then
    STEP_JSON="$(AMUX_ASSISTANT_STEP_SKIP_PRESENT=true "$STEP_SCRIPT" run \
    ...
    STEP_JSON="$(AMUX_ASSISTANT_STEP_SKIP_PRESENT=true "${STEP_ARGS[@]}")" || STEP_EXIT=$?
  fi
...
AMUX_ASSISTANT_MESSAGE="$STATUS_EMOJI $FINAL_SUMMARY"
case "$TURN_VERBOSITY" in
  quiet)
    ;;
  normal)
    if [[ -n "${LAST_NEXT_ACTION// }" ]]; then
      AMUX_ASSISTANT_MESSAGE+=$'\n'"Next: $LAST_NEXT_ACTION"
    fi
...
AMUX_ASSISTANT_PAYLOAD="$(jq -n \
  ...
  --arg suggested_command "$LAST_SUGGESTED_COMMAND" \
  --arg turn_id "$TURN_ID" \
  --arg channel_message "$AMUX_ASSISTANT_MESSAGE" \
...
if [[ "${AMUX_ASSISTANT_TURN_SKIP_PRESENT:-false}" == "true" ]]; then
  printf '%s\n' "$AMUX_ASSISTANT_PAYLOAD"
elif [[ -x "$AMUX_ASSISTANT_PRESENT_SCRIPT" ]]; then
  "$AMUX_ASSISTANT_PRESENT_SCRIPT" <<<"$AMUX_ASSISTANT_PAYLOAD"
else
  printf '%s\n' "$AMUX_ASSISTANT_PAYLOAD"
fi
