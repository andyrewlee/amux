#!/usr/bin/env bash
# assistant-step.sh — Bounded amux step runner for chat/orchestrator flows.
#
# Usage:
...
run_with_deadline() {
  ...
  local out_file
  local exit_file
  out_file="$(mktemp -t assistant-step.out.XXXXXX)"
  exit_file="$(mktemp -t assistant-step.exit.XXXXXX)"
...
redact_secrets_text() {
  local input="$1"
  # Best-effort masking for common token/key patterns before Assistant delivery.
...
AUTO_IDEMPOTENCY="${AMUX_ASSISTANT_STEP_AUTO_IDEMPOTENCY:-true}"
...
if [[ "$STATUS" == "timed_out" && "$SUBSTANTIVE_OUTPUT" != "true" && -n "$SESSION_NAME" ]]; then
  ...
  RECOVERY_POLLS="${AMUX_ASSISTANT_STEP_TIMEOUT_RECOVERY_POLLS:-6}"
  RECOVERY_INTERVAL="${AMUX_ASSISTANT_STEP_TIMEOUT_RECOVERY_INTERVAL:-5}"
  RECOVERY_LINES="${AMUX_ASSISTANT_STEP_TIMEOUT_RECOVERY_LINES:-160}"
...
STEP_VERBOSITY="$(normalize_verbosity_level "${AMUX_ASSISTANT_STEP_VERBOSITY:-normal}")"
STEP_DETAIL_LINES="${AMUX_ASSISTANT_STEP_DETAIL_LINES:-}"
...
AMUX_ASSISTANT_DELTA_EXCERPT=""
if [[ "$STEP_DETAIL_LINES" -gt 0 && -n "${DELTA_COMPACT// }" ]]; then
  AMUX_ASSISTANT_DELTA_EXCERPT="$(build_delta_excerpt "$DELTA_COMPACT" "$STEP_DETAIL_LINES")"
fi
AMUX_ASSISTANT_MESSAGE="$STATUS_EMOJI $SUMMARY"
case "$STEP_VERBOSITY" in
  quiet)
  ...
  normal)
    ...
    if [[ -n "${AMUX_ASSISTANT_DELTA_EXCERPT// }" ]]; then
      AMUX_ASSISTANT_MESSAGE+=$'\n'"Details:"$'\n'"$AMUX_ASSISTANT_DELTA_EXCERPT"
    fi
...
AMUX_ASSISTANT_CHUNK_CHARS="${AMUX_ASSISTANT_STEP_CHUNK_CHARS:-1200}"
if ! [[ "$AMUX_ASSISTANT_CHUNK_CHARS" =~ ^[0-9]+$ ]]; then
  AMUX_ASSISTANT_CHUNK_CHARS=1200
fi
if [[ "$AMUX_ASSISTANT_CHUNK_CHARS" -le 0 ]]; then
  AMUX_ASSISTANT_CHUNK_CHARS=1200
fi
...
if [[ "${AMUX_ASSISTANT_STEP_SKIP_PRESENT:-false}" == "true" ]]; then
