...
If a step times out with no visible output yet, `openclaw-step.sh` (aliasing `openclaw-step.sh`) now performs a short post-timeout capture recovery pass and may set:
...
```bash
scripts/wait-for-idle.sh --session <session_name> [--timeout 300] [--idle-threshold 10]
```
...
```bash
amux --json agent capture <session> --lines 80 | jq -r '.data.content' | scripts/format-capture.sh --strip-ansi --trim
```
